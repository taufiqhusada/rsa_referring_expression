...
class RSA:
    def __init__(self, df, **kwargs):
        ...
        # add top_k_features
        self.top_k_features = kwargs['top_k_features']
...
#       if the word does not appear in the dictionary/vocabulary, treat it as a blank character
    ...
    def create_obj_to_attr_types(self):
        ...
        for obj in dict_obj_to_types_above_threshold:
            ...
            for i in range(min(self.top_k_features, len(dict_obj_to_types_above_threshold[obj]))):
        ...
        # for each object sort the features (based on prob) then pick top_k_features attributes
        for obj in dict_obj_to_attrs_above_threshold:
            dict_obj_to_attrs_above_threshold[obj].sort(reverse=True)
            for i in range(min(self.top_k_features, len(dict_obj_to_attrs_above_threshold[obj]))):
