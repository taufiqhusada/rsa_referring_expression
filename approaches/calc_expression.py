...
def calc_expression(start=0, end=5, k=3, target_set='test', top_k_features=5):
...
#         idx_to_word = tokenizer['idx_to_word']
        ...
        rsa_agent = RSA(df, generated_relations=generated_relations, top_k_features=top_k_features)#,\
...
if __name__ == '__main__':
    ...
    parser.add_argument('--top_k_features', type=int, default=5, help='top k features')
    args = parser.parse_args()
    start, end = args.start, args.end
    k = args.k
    target_set = args.target_set
    top_k_features = args.top_k_features
    calc_expression(start, end, k, target_set, top_k_features)